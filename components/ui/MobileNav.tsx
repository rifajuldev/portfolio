...
const MobileNav: React.FC<MobileNavProps> = ({ setIsMobileNavOpen }) => {
  ...
  return (
    <div
      className="ease-custom-ease-2 bg-neutral-1000 fixed top-0 right-0 z-[1000] min-h-screen w-full max-w-[380px] translate-x-0 translate-y-0 transform transition-all !duration-500"
      style={{ boxShadow: '0 0 15px 0 rgba(0, 0, 0, 0.09)' }}
    >
      <div className="h-full overflow-x-hidden overflow-y-auto p-0 pb-[30px]">
        {/* Nav Heading Start */}
        <div className="flex-between relative mb-[25px] border-b border-neutral-200 px-6 py-5">
          {/* Mobile Nav Logo Start */}
          <Link href="/" className="inline-flex items-center">
            <Image src="/favicon.svg" width={36} height={36} alt="logo" />
            <span className="fs-4 text-neutral-0 ml-2 text-[23px]">Rifaj.dev</span>
          </Link>
          {/* Mobile Nav Logo End */}
          ...
          >
            <span className="absolute left-[10px] hidden h-[1px] w-[20px]"></span>
            <span className="bg-neutral-0 absolute top-[18px] left-[10px] block h-[1px] w-[20px] rotate-45"></span>
            <span className="bg-neutral-0 absolute bottom-[19px] left-[10px] block h-[1px] w-[20px] -rotate-45"></span>
          </button>
          {/* Nav Close Icon End */}
        </div>
        {/* Nav Heading End */}
                ...
                {navItems.map(({ label, route }) => {
                  ...
                  return (
                    <li className="relative mb-5 block p-0" key={route}>
                      <Link
                        href={route}
                        className={`inline-block rounded px-4 py-2 text-base leading-[26px] font-normal transition-all !duration-300 ${
        ...
        <ul className="flex gap-6 px-[30px] text-white md:hidden">
