...
const Contact = () => {
  ...
  return (
    <section id="contact" className="relative overflow-hidden pb-[60px]">
      <div className="items-center lg:flex">
        ...
        <div className="flex flex-col lg:w-5/12 lg:pl-16">
          {combinedContactListData.length > 0 ? (
            combinedContactListData.map(({ id, mediaName, mediaData, link, icon: Icon }) => (
              <div key={id} className="mb-4">
                {mediaName === 'email' ? (
                  ...
                  <Link href={link} target="_blank" className="flex items-center gap-4">
                    ...
                    <div>
                      <span className="text-sm text-neutral-400 capitalize">{mediaName}</span>
                      <h6 className="text-lg break-words">{mediaData}</h6>
                    </div>
                  </Link>
                )}
              </div>
